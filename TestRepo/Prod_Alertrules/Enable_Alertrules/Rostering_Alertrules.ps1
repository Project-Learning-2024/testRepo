...
"Rostering Prod - adf pipeline failure alert"
 ...
 foreach ($rule in $Alertrules)
 {
     ...
     az monitor metrics alert update --enabled true --name "$rule" --resource-group "$rg"
 }

#--------------------------------------------------------------------------------------------------
#--------------------------------------------------------------------------------------------------
#Enabling scheduledqueryrules:
...
foreach ($rule in $Alertrules)
 {
	...
	az monitor scheduled-query update --disabled false --name $rule"" --resource-group "$rg"
 }	
